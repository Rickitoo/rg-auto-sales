...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['upload_fotos'])) {
    ...
    if (!isset($_FILES['fotos']) || empty($_FILES['fotos']['name'][0])) {
        ...
        if ($enviadas > 0) {
            redirect_to('admin/carros/carro_fotos.php?carro_id=' . $carro_id . '&upload=ok');
        } else {
...
<div class="small-container">
    <div class="topbar">
        ...
        <div style="display:flex; gap:10px; flex-wrap:wrap;">
            <a class="btn btn--outline" href="<?= h(url('admin/dashboard.php')) ?>">← Dashboard</a>
            <a class="btn btn--outline" href="<?= h(url('admin/carros/listar_carros.php')) ?>">← Carros</a>
            <a class="btn" href="<?= h(public_url('product-details.php?id=' . $carro_id)) ?>" target="_blank" rel="noopener">Ver no site</a>
        </div>
    </div>
